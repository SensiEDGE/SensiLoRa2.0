...
#define TMPSENSOR_ADCREFVOL  3 /* Typical reference voltage, V  */
...
uint16_t SYS_GetBatteryLevel(void)
{
  ...
  uint32_t l_measuredLevel = 0;

  l_measuredLevel = ADC_ReadChannels(ADC_CHANNEL_3);

  float l_value = (l_measuredLevel * 3.0) / 4095.0;
  float l_inputVolt = l_value / (463.0/(470.0+463.0));

  return l_inputVolt *100;
